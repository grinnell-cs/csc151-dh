...
As we have seen, Scheme uses `cons` to build lists. As you may
recall, `cons` takes two arguments. Up to this point, the first
element has been a value and the second has been a list. When you
call `cons`, Scheme (Racket, DrRacket, something like that) actually
builds a structure in memory with two parts, one of which refers
to the first argument to `cons` and the other of which refers to
the second. This structure is called a cons cell or a pair.

Let us now consider a graphical way to represent the result of a call
to `cons`. The basic idea is to use a rectangle, divided in half,
...
the base cases are the values that are not pairs, and must therefore
...
Most often, people pronounce this operations by reading the letters,pronouncing `caddr` as "see eh dee dee are".  Fortunately, all of these letters equire only one syllable.
