...
> For example, consider the "fancy house" from [the reading on
user-defined procedures](../readings/procedures.html).  We might 
call that `my-image`.

> We wrote a `(fancy-house size color)` procedure in
[the corresponding lab](../labs/procedures.html).  Depending on
how we defined that procedure, we would write something like
`(define my-image-alt (fancy-house 50 "black"))` to get another
house that looks identical to `my-image`.

> But the point of `fancy-house` isn't just that we can make an
identical house.  We should also be able to make variants.  So we
might write `(define my-other-image (fancy-house 100 "yellow"))` to
show that we can make different size and diffent color houses.
